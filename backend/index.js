...
app.post("/login/submit", (request, response) => {
  ...
  if (!username || !password)
    return response.status(400).send({ error: "Username or Password Fild is Empty!" });
...
app.post("/addTask", (request, response) => {
  ...
  if (duplicateCheck) return response.send({ message: "Duplicate" }).status(400);
...
app.patch("/moveRight/:id", (request, response) => {
  const id = Number(request.params.id);
...
app.patch("/moveLeft/:id", (request, response) => {
  const id = Number(request.params.id);
